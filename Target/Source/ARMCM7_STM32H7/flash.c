...
#define FLASH_WRITE_BLOCK_SIZE          (128u * 1024u)
...
#define BOOT_FLASH_VECTOR_TABLE_CS_OFFSET (126 * 4) /* 126 interrupts * 4 bytes per address */
#endif
...
{
/*{ 0x08000000 +  0u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE,  0},    flash sector  0 - reserved for bootloader */
  { 0x08000000 +  1u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE,  1}, /* flash sector  1 - 128kb                   */
  { 0x08000000 +  2u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE,  2}, /* flash sector  2 - 128kb                   */
  { 0x08000000 +  3u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE,  3}, /* flash sector  3 - 128kb                   */
  { 0x08000000 +  4u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE,  4}, /* flash sector  4 - 128kb                   */
  { 0x08000000 +  5u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE,  5}, /* flash sector  5 - 128kb                   */
  { 0x08000000 +  6u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE,  6}, /* flash sector  6 - 128kb                   */
  { 0x08000000 +  7u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE,  7}, /* flash sector  7 - 128kb                   */
#if (BOOT_NVM_SIZE_KB > 1024)
  { 0x08000000 +  8u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE,  8}, /* flash sector  8 - 128kb                   */
  { 0x08000000 +  9u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE,  9}, /* flash sector  9 - 128kb                   */
  { 0x08000000 + 10u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE, 10}, /* flash sector 10 - 128kb                   */
  { 0x08000000 + 11u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE, 11}, /* flash sector 11 - 128kb                   */
  { 0x08000000 + 12u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE, 12}, /* flash sector 12 - 128kb                   */
  { 0x08000000 + 13u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE, 13}, /* flash sector 13 - 128kb                   */
  { 0x08000000 + 14u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE, 14}, /* flash sector 14 - 128kb                   */
  { 0x08000000 + 15u * FLASH_WRITE_BLOCK_SIZE, FLASH_WRITE_BLOCK_SIZE, 15}, /* flash sector 15 - 128kb                   */
#endif
 ...
 *           overhead, yet results in faster flash programming because data is first
 ...
 *           sequence, the checksum. This means that this flash block needs to be written
...
**
****************************************************************************************/
static void CpuMemCopy32(blt_addr dest, blt_addr src, blt_int32u len)
{
  blt_int8u *from, *to;
...
static blt_bool FlashInitBlock(tFlashBlockInfo *block, blt_addr address)
{
  ...
  CpuMemCopy32((blt_addr)block->data, address, FLASH_WRITE_BLOCK_SIZE);
  return BLT_TRUE;
...
static blt_bool FlashWriteBlock(tFlashBlockInfo *block)
{
  ...
  /* HAL_FLASH_Program writes 256 bits at a time to the flash. */
  const blt_int32u flashword_size = 32;    /* in bytes */

  /* program the "flash words" in the block one by one */
  for (blt_int32u word_cnt = 0; word_cnt < (FLASH_WRITE_BLOCK_SIZE/flashword_size); word_cnt++)
  {
    const blt_addr prog_addr = block->base_addr + (word_cnt * flashword_size);
    const blt_int32u* prog_data = (blt_int32u*)(&block->data[word_cnt * flashword_size]);

    /* keep the watchdog happy */
    CopService();

    /* use a temporary buffer to ensure that the data to be flashed satisfies
     * the required 4-byte alignment. */
    uint32_t buffer[8];
    ASSERT_RT(sizeof(buffer) == flashword_size);
    memcpy(&buffer, prog_data, sizeof(buffer));

    /* program the flash word */
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, prog_addr, (uint32_t)&buffer[0]) != HAL_OK)
    {
      result = BLT_FALSE;
      break;
    }
    ...
    if (memcmp(prog_data, (const void *)prog_addr, flashword_size) != 0)
    {
      result = BLT_FALSE;
      break;
    }
